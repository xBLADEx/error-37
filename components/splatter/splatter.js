  ...
  useEffect(() => {
    ...
    function splat(x, y, arr) {
      for (var i = 0; i < 30; i++) {
        ...
        var dirx = (Math.random() < 0.5 ? 3 : -3) * (Math.random() * 3) * options.scatter;
        var diry = (Math.random() < 0.5 ? 3 : -3) * (Math.random() * 3) * options.scatter;
    ...
    canvas.onmousedown = function (e) {
      if (!focused) {
        ...
        var redtone = options.shade
          ? 'rgb(' + (130 + ((Math.random() * 105) | 0)) + ',0,0)'
          : '#aa0707';
